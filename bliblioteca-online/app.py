...
def foto():
    if request.method == 'POST':
        imga = request.form['imagem']

        conn = psycopg2.connect(**db_config)
        cursor = conn.cursor()

        cursor.execute("INSERT INTO img (imagem) VALUES (%s)", (imga,))
...
@app.route('/cimg', methods=['GET', 'POST'])
def cimg():
    if request.method == 'POST':
        relatorio = request.form['relatorio']
        conexao = psycopg2.connect(**db_config)
        cursor = conexao.cursor()
        cursor.execute("SELECT imagem FROM img")
        result = cursor.fetchall()

        conexao.close()  # Fechando a conexão
        return render_template('img.html', result=result)
    else:
        return render_template('img.html', result=None)
